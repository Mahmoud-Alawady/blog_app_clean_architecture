...
class _BlogPageState extends State<BlogPage> {
  ...
  void initState() {
    super.initState();
    context.read<BlogFetchBloc>().add(BlogFetchEventGetAllBlogs());
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: BlocConsumer<BlogFetchBloc, BlogFetchState>(
          listener: (context, state) {
            if (state is BlogFetchFailure) {
              showSnackBar(context, state.error);
            }
          },
          builder: (context, state) {
            if (state is BlogFetchLoading) return const Loader();
